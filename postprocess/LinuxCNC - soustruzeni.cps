/**
  ...
  $Date: 2019-09-20 06:29:14 $
  
  FORKID {D3E630A8-AFCC-46E6-BEF1-6AD5A6FA5483}
*/

description = "LinuxCNC - Soustruzeni";
vendor = "LinuxCNC";
...
properties = {
  ...
  pathTolerance: 0.001, // maximal tolerance
  ...
  maximumSpindleSpeed: 5000, // specifies the maximum spindle speed, 5C high speed 3500rpm, D1-4 low speed 2500rpm
...
propertyDefinitions = {
  ...
  pathTolerance: {title: "Presnost", description: "Neodchylit se vice nez dana vzdalenost", group: 0, type:"number"},
...
var pFormat = createFormat({decimals:(unit == MM ? 3 : 4), forceDecimal:true});
...
function commentSafe(strAccents) {
  var strAccents = strAccents.toLowerCase().split('');
  var strAccentsOut = new Array();
  var strAccentsLen = strAccents.length;
  var accents =    'ěščřžýáíéúůť()';
  var accentsOut = "escrzyaieuut  ";
  ...
  strAccentsOut = strAccentsOut.join('').toUpperCase().replace(/[()]/g, "");
  return strAccentsOut;
}

function formatComment(text) {
  return "(" + commentSafe(String(text)) + ")";
}
...
function onOpen() {
  ...
  writeBlock(gFormat.format(64), pOutput.format(properties.pathTolerance)) // Path tolerance
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    writeBlock(gFormat.format(54), gMotionModal.format(0), yOutput.format(0));
    if (tool.comment) {
